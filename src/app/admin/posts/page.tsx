    ...
    setTimeout(() => {
      const dummyPosts: AdminPostView[] = [
        ...
        },
        {
          ...
          media: [
            { type: 'image', url: '/placeholder-image.jpg', publicId: 'sample1' }
          ]
        }
      ];
      ...
      <Container maxWidth="lg">
                  ...
                  <TableRow key={post._id} hover>
                    <TableCell>
                      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                        {getStatusChip(post)}
                        {post.media && (
                          <Chip label="メディア" variant="outlined" size="small" />
                        )}
                      </Box>
                    </TableCell>
        ...
        >
          ...
          {session?.user?.role === 'admin' && (
            <MenuItem onClick={() => handleModeration('delete', selectedPost!)} sx={{ color: 'error.main' }}>
              <DeleteIcon sx={{ mr: 1 }} />
              削除
            </MenuItem>
          )}
        ...
        {/* モデレーション確認ダイアログ */}
        <Dialog open={Boolean(moderationDialog)} onClose={() => setModerationDialog(null)} maxWidth="sm" fullWidth>
          <DialogTitle>
            投稿モデレーション - {moderationDialog === 'hide' ? '非表示' : moderationDialog === 'delete' ? '削除' : '操作'}
          </DialogTitle>
          <DialogContent>
            {selectedPost && (
              <Box>
