...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!validateObjectId(id)) {
      console.warn('🚨 無効なObjectID検出:', { id, ip });
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // ObjectID の検証強化
    if (!validateObjectId(id)) {
      console.warn('🚨 無効なObjectID検出（PUT）:', { id, ip });
      return NextResponse.json({ error: '無効な投稿IDです' }, { status: 400 });
    }
    ...
    if (detectXSSAttempt(content) || (title && detectXSSAttempt(title))) {
      ...
      return NextResponse.json({ error: '不正なコンテンツが検出されました' }, { status: 400 });
    }
    ...
    if (content.length > 1000) {
      return NextResponse.json({ error: '投稿は1000文字以内で入力してください' }, { status: 400 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    // ObjectID の検証強化
    if (!validateObjectId(id)) {
      console.warn('🚨 無効なObjectID検出（DELETE）:', { id, ip });
      return NextResponse.json({ error: '無効な投稿IDです' }, { status: 400 });
    }
