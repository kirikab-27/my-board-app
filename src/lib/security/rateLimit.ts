...
export function checkUserRateLimit(email: string): {
  ...
  if (remainingAttempts <= 0) {
    ...
    const lockoutMs =
      SECURITY_CONFIG.PROGRESSIVE_LOCKOUT[
        lockoutIndex as keyof typeof SECURITY_CONFIG.PROGRESSIVE_LOCKOUT
      ] || SECURITY_CONFIG.PROGRESSIVE_LOCKOUT[4];
...
export function getRateLimitInfo(ip: string, email?: string) {
  ...
  return {
    ip: {
      ...
      remaining: ipRecord
        ? SECURITY_CONFIG.IP_LIMIT.maxAttempts - ipRecord.attempts
        : SECURITY_CONFIG.IP_LIMIT.maxAttempts,
    },
    user: userRecord
      ? {
