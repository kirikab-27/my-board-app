...
import { RateLimiter } from '../lib/verification/rateLimiter';
...
class VerificationSystemTester {
  ...
  private async testCodeGeneration(): Promise<void> {
    ...
    try {
      ...
      console.log(`${passed ? '✅' : '❌'} コード生成テスト: ${result.success ? 'SUCCESS' : 'FAILED'}`);
  ...
  private async testCodeVerification(): Promise<void> {
    ...
    try {
      ...
      console.log(`${passed ? '✅' : '❌'} コード検証テスト: 正解=${verifyResult.success}, 不正解=${!wrongCodeResult.success}`);
  ...
  private async testRateLimit(): Promise<void> {
    ...
    try {
      ...
      // 複数回リクエストして制限を確認
      const requests = [];
      for (let i = 0; i < 6; i++) { // メール制限（5回）を超える
        requests.push(
      ...
      const rateLimitCount = results.filter(r => !r.success && r.error?.includes('レート制限')).length;
      ...
      console.log(`${passed ? '✅' : '❌'} レート制限テスト: ${successCount}成功, ${rateLimitCount}制限`);
  ...
  private async testBruteForceProtection(): Promise<void> {
    ...
    try {
      ...
      console.log(`${passed ? '✅' : '❌'} ブルートフォース対策テスト: ${lockedResults.length}ロック`);
  ...
  private async testInputValidation(): Promise<void> {
    ...
    try {
      ...
      console.log(`${allPassed ? '✅' : '❌'} 入力バリデーションテスト: ${results.filter(r => r.passed).length}/${testCases.length}`);
  ...
  private async testConcurrentGeneration(): Promise<void> {
    ...
    try {
      ...
      console.log(`${passed ? '✅' : '❌'} 同時実行テスト: ${successCount}成功, ${uniqueCodes}ユニーク`);
  ...
  private async testDatabaseCleanup(): Promise<void> {
    ...
    try {
      ...
      console.log(`${passed ? '✅' : '❌'} データベースクリーンアップテスト: ${cleanupResult.deletedCount}削除`);
  ...
  private async printResults(): Promise<void> {
    ...
    this.results.forEach(result => {
      ...
      console.log(`${status} ${result.testName.padEnd(30)} ${result.duration.toString().padStart(4)}ms`);
