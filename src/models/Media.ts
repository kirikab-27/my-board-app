...
const MediaSchema: Schema = new Schema(
  {
    ...
    metadata: {
      ...
      exif: {
        ...
        shutterSpeed: {
          type: String,
          maxlength: [20, 'シャッタースピードは20文字以内である必要があります'],
        },
    ...
    cloudinary: {
      ...
      transformations: [
        { type: String, maxlength: [100, '変換設定は100文字以内である必要があります'] },
      ],
    ...
    security: {
      ...
      scanEngine: {
        type: String,
        maxlength: [50, 'スキャンエンジン名は50文字以内である必要があります'],
      },
    },
...
// 統計更新メソッド
MediaSchema.methods.updateStats = async function (
...
MediaSchema.methods.scanForSecurity = async function (): Promise<void> {
  try {
    ...
    // ファイル名チェック
    if (
      this.filename.toLowerCase().includes('virus') ||
...
MediaSchema.statics.getPopularMedia = async function (options: any = {}): Promise<IMedia[]> {
  ...
  return this.find(query).sort({ 'stats.views': -1, 'stats.shares': -1 }).limit(limit);
};
