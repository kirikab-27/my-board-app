...
// 検証試行結果の定義
export type AttemptResult = 'success' | 'invalid_code' | 'expired' | 'locked' | 'rate_limited' | 'used';
...
// スタティックメソッド
VerificationAttemptSchema.statics.getFailureRate = function(email: string, type: VerificationType, hours: number = 1) {
...
VerificationAttemptSchema.post('save', function(doc) {
  // 高リスクスコアのアラート
  if (doc.riskScore > 80) {
    console.warn(`🚨 高リスク認証試行: ${doc.email} (Risk: ${doc.riskScore}, IP: ${doc.ipAddress})`);
  }
...
const VerificationAttempt = mongoose.models.VerificationAttempt || 
